/*
  # Fix user sessions policy

  1. Changes
    - Recreates the recent_user_logins view
    - Drops existing policy before creating a new one
    - Ensures RLS is enabled on user_sessions table
    - Creates a policy that allows admin users to see all sessions
    - Allows regular users to only see their own sessions
  
  2. Security
    - Uses SECURITY INVOKER for the view
    - Properly handles type casting between UUID and bigint
*/
...
DROP POLICY IF EXISTS "Only admin can access recent logins" ON public.user_sessions;
